...
加载网页的第一步就是 *Navigation#导航* ，即用户 输入 URL 回车, 或者点击链接。
...
一旦知道了 IP 地址，浏览器将会通过一个 TCP 三次握手以建立一个连接在 目标服务器和客户端浏览器之间。 

TCP 三次握手的过程以 SYN , SYN-ACK , ACK 标识，这三此消息用于协商并在两台计算机之间启动TCP会话。



### 1.3 TLS Negotiation#协商

如果是 HTTPS， 还需要 另一次 “握手”。 这个握手更确切的说是 TLS 协商，决定了后续的通信以什么加密算法进行加密， 验证服务器，并在真正传输数据前建立安全连接。 这要求在网页内容请求真正被发送前，还需要三轮往返协商。 
...
虽然使连接安全会增加页面加载的时间，但安全连接的延迟开销是值得的，因为在浏览器和web服务器之间传输的数据无法被第三方解密。
...
一旦和 web 服务器建立了连接，浏览器会代表用户发起一个 **初始化 HTTP GET 请求**，对于网站来说，通常是一个 HTML 文件。 一旦服务器接收到了请求，它会返回相关的响应头以及 HTML 的内容。 
...
**第一步**，处理 HTML 标记，并构建 DOM 树。 HTML 解析包含了  [tokenization](https://developer.mozilla.org/en-US/docs/Web/API/DOMTokenList) and tree construction. 
...
当浏览器构建DOM 树的时候，这个过程将会占用主线程， 这时候， *preload scanner* 将会解析可用的内容，请提高 CSS， JavaScript， 字体等资源的请求优先级。多亏了 *preload scanner*， 我们才不用必须等到解析器找到外部资源的引用才请求它，它将会在后台检索资源，这样，当主HTML解析器到达所请求的资源时，它们可能已经在运行中了，或者已经被下载。 预加载扫描器提供的优化，减少了页面阻塞。
